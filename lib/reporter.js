...
function getClientData(eventStr) {
  const startClientData = eventStr.indexOf('\"clientData\":{');
  if (startClientData >= 0) {
    return JSON.parse(eventStr.substring(startClientData + 14, eventStr.length - 1));
  }
}
        ...
        if (event) {
          ...
          if (eventCounter[index] % config.eventStep === 0 &&
            ...
            if (!clientData) {
              ...
              let doProcessEvent = true;
              if (clientData.isTester) {
                if (clientData.name === 'page-visited') {
                  clientData.name = 'page-visited::tester';
                }
                else {
              ...
              if (doProcessEvent) {
                ...
                clientData.added = {}; // Room for our added meta data (see added-info/)
