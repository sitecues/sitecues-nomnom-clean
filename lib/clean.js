...
const fs = require('fs'),
  ...
  addMetaData = require('./metadata/metadata'),
  missingDays = [],
  eventCounter = [];
...
function findRawFile(fileName, filePath) {
  try {
    ...
    return fetchMissingData(fileName, filePath)
      ...
      .catch(() => {
        logger('Failed to download ' + fileName);
  ...
  return new Promise((resolve) => {
    ...
    input
      ...
      .pipe(es.map((event, callback) => {
        if (event) {
          ...
          const allEventData = JSON.parse(event),
            ...
            backupSiteId = allEventData.siteId;
          if (!clientData) {
      ...
      .on('finish', () => {
        ...
        fs.renameSync(tmpDataPath, cleanDataPath, resolve);
      });
  });
}
    ...
    function addToQueue() {
      if (-- nextUp >= 0) {
        ...
        const logFileName = logFileNames[nextUp],
          rawDataPath = path.join(config.dataFolder, constants.RAW_DATA_SUBFOLDER, logFileName),
          cleanDataPath = path.join(config.dataFolder, constants.CLEAN_DATA_SUBFOLDER, logFileName),
          nowUp = nextUp;
