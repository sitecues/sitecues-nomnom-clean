...
function getDatesArray(startDate, endDate, dayStep) {
  var start = new Date(startDate || constants.BEGINNING_OF_TIME),
...
function init(options) {
  const reports = getReports(options.reports) || getAllReports(),
    dates = getDatesArray(getDateAsString(options.startDate), getDateAsString(options.endDate), options.dayStep || 1),
