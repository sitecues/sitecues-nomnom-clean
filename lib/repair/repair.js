...
function fixParsedEvent(parsedEvent, backupSiteId, serverTs, eventId, dateIndex) {
  // Field names
  ...
  if (parsedEvent.userId && (parsedEvent.scVersion === '4.2.7-RELEASE' || parsedEvent.scVersion === '4.2.8-RELEASE')) {
    // Create a session ID unique to this user and this domain
    const newSessionId = parsedEvent.userId + '-' + parsedEvent.meta.domain + '-' + dateIndex;
