...
// function getFakeAbTestSeed(id) {
//   const parts = id.split('-'),
//     firstPart = parts[1];
//   return parseInt(firstPart, 16);
// }

// function addFakeAbTest(parsedEvent, dateIndex) {
...
//   // Only 1 in 8 users gets the test
//   const userSeed = getFakeAbTestSeed(userId);
//   if ((userSeed % 8) > 0) {
//     return;
//   }

//   // Base fake results on site key in the hopes of getting different results for each
//   const siteSeed = getFakeAbTestSeed(parsedEvent.siteId);
//   if (abTest === 'veggie.') {
//     abTest += [ 'carrot', 'potato', 'corn', 'beet', 'pea', 'asparagus'][siteSeed % 6];
//   }
//   else {
//     abTest += [ 'grape', 'apple', 'banana'][siteSeed % 3];
//   }

//   parsedEvent.abTest = abTest;
// }
    ...
    input
      ...
      .pipe(es.map((event, callback) => {
        if (event) {
          ...
          else if (repairEvent(clientData, backupSiteId, allEventData.serverTs, allEventData.eventId)) {
            // addFakeAbTest(clientData, dateIndex); // TODO remove this once we have real AB test data
...
function deleteOldFiles(deletePattern) {
  try {
    logger('Deleting temporary files ... ');
    const pattern = path.join(config.cleanDataFolder, deletePattern);
...
function run(options) {
  ...
  deleteOldFiles(config.doClean ? '*.gz' : '*.tmp.gz');
