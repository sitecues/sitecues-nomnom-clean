...
function getUaInfo(parsedEvent) {
  ...
  function getBrowserVersion() {
    ...
    function getNumberAfterString(str) {
      const charIndex = usefulPart.lastIndexOf(str);
      if (charIndex < 0) {
        return 0;
        // throw new Error('Unexpected user agent string format -- can\'t find browser version for ' + browser + ':\n' + uaString);
      }
      return parseInt(usefulPart.substr(charIndex + str.length));
    }
  ...
  if (!uaString.startsWith(PREFIX)) {
    return {
      browser: 'bad UA format',
      isUnsupported: true
    };
  }

  const usefulPart = uaString.substring(OS_OFFSET),
    possibleUserAgents = usefulPart.startsWith(QUICK_CHECK_WIN) ? winUserAgentNames :
      (usefulPart.startsWith(QUICK_CHECK_OSX) ? macUserAgentNames : rejectedOSUserAgentNames),
    browser = getMatchingUserAgent(usefulPart, possibleUserAgents),
    browserInfo = userAgentInfo[browser],
    minVersion = browserInfo.minVersion,
    browserVersion = minVersion ? getBrowserVersion(usefulPart, browser) : undefined,
    isUnsupported =  !browserVersion || browserVersion < minVersion || undefined,
