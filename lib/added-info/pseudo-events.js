...
function getZoomChangedTypes(event) {
  ...
  if (details.isSlider) {
    ...
    pseudoEvents.push(details.isLongGlide ? 'via-a-button-glide' : 'via-a-button');
  }
  else if (details.isKey) {
    pseudoEvents.push(details.isLongGlide ? 'via-key-glide' : 'via-key');
    pseudoEvents.push(details.isBrowserKeyOverride ? 'via-browser-key' : 'via-simple-key');
  }
  else {
