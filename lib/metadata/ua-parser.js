...
function getUaInfo(parsedEvent) {
  ...
  function getBrowserGroups(isUnsupported, browser, os, browserInfo) {
    const groups = browserInfo.groups.concat([
      '@any',
    ...
    return groups.concat('@firefox-or-chrome-on' + os);
  }
  ...
  const
    ...
    groups = getBrowserGroups(isUnsupported, browser, os, browserInfo);
