...
function getPageVisitPseudoEvents(parsedEvent, ua) {
  const
    isNativeZoomOn = parsedEvent.details && parsedEvent.details.nativeZoom > 1.02,
  ...
  if (isNativeZoomOn) {
    pseudoEvents.push('native-zoom-on');
  }
...
function getPanelClickedPseudoEvents(parsedEvent) {
  return parsedEvent.details.target && [ parsedEvent.details.target ];
}
...
function getPseudoEvents(event, ua) {
  ...
  if (pseudoEventsGetterFn) {
    const events = pseudoEventsGetterFn(event, ua);
    return events && events.map((pseudoEvent) => event.name + '::' + pseudoEvent);
  }
}
