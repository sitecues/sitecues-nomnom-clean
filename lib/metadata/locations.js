...
function getBadgeGroups(parsedEvent) {
  ...
  let badgeGroups = [
    '@pagebadge',
    '@pagebadge-palette-' + parsedEvent.badgePalette,
    '@pagebadge-height-' + parsedEvent.badgeHeight
  ];

  if (isLeft) {
    badgeGroups = badgeGroups.concat('@pagebadge-pos-left');
  }
  if (isRight) {
    badgeGroups = badgeGroups.concat('@pagebadge-pos-right');
  }
  if (isTop) {
    badgeGroups = badgeGroups.concat('@pagebadge-pos-top');
  }
  if (isBottom) {
    badgeGroups = badgeGroups.concat('@pagebadge-pos-bottom');
  }
  return badgeGroups;
}
