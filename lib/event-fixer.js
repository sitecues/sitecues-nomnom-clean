...
function fixParsedEvent(parsedEvent) {
  ...
  if (parsedEvent.metricsVersion) {
    parsedEvent.metricVersion = parsedEvent.metricsVersion;
    delete parsedEvent.metricsVersion;
  }

  if (parsedEvent.name === 'panel-clicked') {
    const origTarget = parsedEvent.details.target;
